...
import { 
  ...
  Activity, SupportTicket, Subscription, PlaybookInstance, Milestone 
} from '../types';
...
export const AccountDetail: React.FC = () => {
  ...
  if (!acc) return <div className="p-12 text-center animate-pulse">Syncing Entity 360...</div>;
                           ...
                           <div className="grid grid-cols-2 gap-8">
                              <div className="p-8 bg-slate-50 rounded-[2rem] border border-slate-100">
                                 ...
                                 <div className="space-y-5">
                                    <div className="flex items-center justify-between"><span className="text-xs font-bold text-slate-500">Sales Owner</span><span className="text-xs font-black text-slate-900">{acc.salesOwner}</span></div>
                                    <div className="flex items-center justify-between"><span className="text-xs font-bold text-slate-500">Delivery Lead</span><span className="text-xs font-black text-slate-900">{acc.deliveryOwner}</span></div>
                                    <div className="flex items-center justify-between"><span className="text-xs font-bold text-slate-500">CS Champion</span><span className="text-xs font-black text-indigo-600">{acc.csOwner}</span></div>
                                 </div>
                              </div>
                              <div className="p-8 bg-slate-50 rounded-[2rem] border border-slate-100">
                                 <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-6">Financial Specs</h4>
                                 <div className="space-y-5">
                                    <div className="flex items-center justify-between"><span className="text-xs font-bold text-slate-500">Renewal Date</span><span className="text-xs font-black text-slate-900">{acc.renewalDate}</span></div>
                                    <div className="flex items-center justify-between"><span className="text-xs font-bold text-slate-500">Plan Tier</span><span className="text-xs font-black text-slate-900">{acc.plan}</span></div>
                          ...
                          {contacts.map(contact => (
                             <div key={contact.id} className="p-8 bg-white border border-slate-200 rounded-3xl shadow-sm hover:border-indigo-400 hover:shadow-xl transition-all group">
                                <div className="flex justify-between items-start mb-6">
                                   <div className="flex items-center space-x-4">
                                      <div className="h-14 w-14 bg-slate-100 rounded-2xl flex items-center justify-center font-black text-slate-500 text-xl group-hover:bg-indigo-600 group-hover:text-white transition-colors">{contact.firstName[0]}</div>
                                      <div>
                                ...
                                <div className="flex flex-col space-y-3 pt-6 border-t border-slate-50">
                                   ...
                                   <div className="flex items-center space-x-3 text-xs text-slate-600 font-bold"><Phone size={14} className="text-slate-300"/> <span>{contact.phone}</span></div>
                                </div>
                             </div>
                          ))}
                       </div>
                 ...
                 {activeTab === 'playbooks' && (
                    <div className="space-y-8 animate-in slide-in-from-bottom-4">
                       ...
                       <div className="grid grid-cols-1 gap-6">
                          {/* Placeholder for actual playbook logic */}
