...
import { 
  Search, Book, ChevronRight, Layout, Database, 
  Layers, Workflow, List, Target, 
  BarChart2, Zap, RotateCcw, MessageSquare, 
  Calendar, Award, ClipboardCheck, ShieldCheck, TrendingUp, X, 
  FileText, Info, ShieldAlert, Sparkles, Languages, CheckCircle2,
  GitBranch, Link2, Coins, UserCheck, Shield, ClipboardList, Play
} from 'lucide-react';
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={auto.id} className="flex space-x-4 p-4 bg-white border border-slate-200 rounded-xl hover:shadow-md transition-all">
                <div className="font-black text-slate-300 text-lg">{auto.id}</div>
                <div>
        ...
        <div className="space-y-8">
          <section className="space-y-4">
            <div className="p-6 bg-slate-900 rounded-3xl text-white">
               <h4 className="text-sm font-black text-blue-400 uppercase tracking-widest mb-4">Scenario 1: Lead-to-Cash (The Happy Path)</h4>
               <ol className="text-xs space-y-3 opacity-80">
                  <li>1. **Intake**: Create a Lead with "alex@acme.com". A1 should link it to Acme Corp.</li>
                  <li>2. **Qualify**: Change Lead status to "Qualified". A2 should spawn a Discovery Deal.</li>
                  <li>3. **Win**: Move Deal to "Closed Won". A4 should create a Delivery project + 3 Milestones.</li>
                  <li>4. **Go-Live**: Move Delivery to "Live". A5 should set Account lifecycle to "Active".</li>
               </ol>
            </div>

            <div className="p-6 bg-slate-50 border border-slate-200 rounded-3xl">
               <h4 className="text-sm font-black text-rose-600 uppercase tracking-widest mb-4">Scenario 2: Critical Risk Handling</h4>
               <ol className="text-xs space-y-3 text-slate-600">
                  <li>1. **Issue**: Open a Support Ticket with severity "Critical".</li>
                  <li>2. **Reaction**: A6 triggers. Go to Account page. Health score should drop (e.g. 90 -> 65).</li>
                  <li>3. **Status**: Lifecycle should flip to "At Risk" if score < 50.</li>
               </ol>
            </div>

            <div className="p-6 bg-indigo-50 border border-indigo-100 rounded-3xl">
               <h4 className="text-sm font-black text-indigo-600 uppercase tracking-widest mb-4">Scenario 3: Financial Blockage</h4>
               <ol className="text-xs space-y-3 text-indigo-900/60">
                  <li>1. **Failure**: Set a Subscription status to "Past Due".</li>
                  <li>2. **Intervention**: A7 triggers. Check Delivery list. The account's project is now "Blocked".</li>
                  <li>3. **Health**: A8 recalculates audit. Score takes a massive -40 hit.</li>
               </ol>
            </div>
          </section>
        </div>
        ...
        {filteredPages.map((page) => (
          ...
          >
            <div className="flex items-center justify-between mb-6">
              ...
              <span className="text-[10px] font-black text-slate-300 group-hover:text-blue-300 tracking-tighter uppercase">{page.category}</span>
            </div>
            <h3 className="text-xl font-black text-slate-900 mb-3 group-hover:text-blue-600 transition-colors leading-tight">{page.title}</h3>
            <p className="text-sm text-slate-500 leading-relaxed mb-6 flex-1">Protocol ID: {page.id.toUpperCase()}</p>
          ...
          <div className="bg-white w-full max-w-4xl max-h-[90vh] rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-300">
            <div className="p-10 border-b border-slate-100 flex justify-between items-center bg-slate-50/30">
              <div className="flex items-center space-x-6">
                ...
                <div>
                  <h3 className="text-3xl font-black text-slate-900">{selectedPage.title}</h3>
                  <p className="text-xs font-black text-blue-600 uppercase tracking-[0.2em]">{selectedPage.category} Standard</p>
                </div>
              </div>
              <button 
            ...
            <div className="flex-1 overflow-y-auto p-12 custom-scrollbar text-slate-700 leading-relaxed">
            ...
            <div className="p-10 border-t border-slate-100 bg-slate-50/50 flex justify-between items-center">
                ...
                <button 
                  onClick={() => setSelectedPage(null)}
                  className="px-10 py-4 bg-slate-900 text-white rounded-2xl font-black text-sm hover:bg-slate-800 transition-all shadow-xl"
                >
                    Dismiss
                </button>
            </div>
